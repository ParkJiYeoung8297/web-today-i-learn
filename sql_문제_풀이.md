...
- crew_id와 nickname이 중복되고 있다.


2. attendance 테이블에서 중복을 제거하기 위해 crew 테이블을 만들려고 한다. 어떻게 구성해 볼 수 있을까?
- crew_id(PK),nickname으로 구성할 수 있다.

3. crew 테이블에 들어가야 할 크루들의 정보는 어떻게 추출할까? (hint: DISTINCT)
```sql
SELECT Distinct crew_id, nickname FROM woowa_tech.attendance;
```
...
FROM woowa_tech.attendance;
```
*직접 값을 넣을 떄는 Insert into Values이지만, select로 조회해서 데이터를 넣을 때는 VALUES를 사용하지 않는다. 

---

### 문제 2: 테이블 컬럼 삭제하기 (ALTER TABLE)
1. crew 테이블을 만들고 중복을 제거했다. attendance에서 불필요해지는 컬럼은?
- crew_id와 nickname이 불필요하다.
...
```
*MYSQL에서는 여러개의 column을 drop하기 위해서 DROP COLUMN을 각각 써야 한다. (여기서는 사용안됨)   

---

### 문제 3: 외래키 설정하기
1. attendance에서 관심사의 분리를 통해 crew 테이블을 별도로 만들었다. 따라서, 나중에 nickname이 필요하다면 crew 테이블에서 확인하면 된다.
...
```sql
INSERT INTO crew (id,nickname) VALUES (13,"어셔");

INSERT INTO attendance (crew_id,attendance_date,start_time,end_time) VALUES
((SELECT id FROM crew WHERE nickname="어셔"), "2025-03-06", "09:31:00", "18:01:00");
```
...
WHERE crew_id = (
    SELECT id FROM crew WHERE nickname = "주니"
) 
...
WHERE crew_id = (
    SELECT id FROM crew WHERE nickname = "아론"
)
AND attendance_date = "2025-03-12";
```
...
ORDER BY attendance_count;
```
*FROM → JOIN → WHERE → GROUP BY → ORDER BY 이므로 JOIN을 WHERE보다 먼저 사용해야한다.
...
```sql
SELECT a.attendance_date, COUNT(*) as attendance_count 
FROM attendance a
JOIN crew c on a.crew_id=c.id
